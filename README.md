...
$ bundle init
$ vim Gemfile
(gem "rails", "6.0.3.4"に書き換え、コメントアウト解除）
...
00:02 deploy:check:make_linked_dirs
      01 mkdir -p /var/rails/sample6034/shared/config
    ✔ 01 ec2-user@52.195.2.51 0.078s
00:02 deploy:check:linked_files
      ERROR linked file /var/rails/sample6034/shared/config/master.key does not exist on 52.195.2.51
```
ここで失敗。エラーは「master.key」がないことが原因。

```bash
$ cd /var/rails/sample6034/shared/config/
$ touch master.key
$ vim master.key
（master.keyを貼り付け）
```
...
```bash
$ cd /var/rails/sample6034/releases/(最新リリース)/
...
```
upstream railsapp {
    server unix:///var/rails/sample6034/current/tmp/sockets/puma.sock fail_timeout=0;
}
...
server {
    ...
    root /var/rails/sample6034/current/public;
...
WorkingDirectory=/var/rails/sample6034/current
ExecStart=/home/ec2-user/.rbenv/shims/bundle exec /var/rails/sample6034/shared/bundle/ruby/2.6.0/bin/puma -C /var/rails/sample6034/current/config/puma/production.rb
Restart=always
